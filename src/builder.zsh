#!/bin/zsh
# MAC OS X Z SHELL SCRIPT FOR BUILDING CERBERUS X TOOLS
...
[ ! "$(uname -o)" = "Darwin" ] && {
    ...
    do_error "This is the macOS version of the builder script. Please use the one meant for your operating system"
    exit 0;
}
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo -e "\t{-b|--bundle}\t\t\t\t\t- Set the application bundle identifier prefix. Default is com.cerberus-x." 
            ...
            echo -e "\t{-p|--pause}\t\t\t\t\t- Pause just before showing the menu just to show script configuration setup." 
...
do_items(){
    ...
    [ $TRANSCC_EXE -eq 1 ] && {
        DISPLAY_ITEMS+=("CServer" "Makedocs" "Launcher" "Clean tool build directories")
        MENU_ITEMS+=("do_cserver" "do_makedocs" "do_launcher" "do_clean");
    }
    [ ${#QT_INSTALLS[@]} -gt 0 ] && {
