...
#            should be ${app_name}-mime-icon-${size}.png and the xdg-icon-resource "icon-name"
...
# ICON_STORE_DIR is the location where converted svg image are stored.
ICON_STORE_DIR="$CERBERUS_ROOT_DIR/share/images/cerberus/hicolor"
...
do_generate_setup_launcher(){
    ...
    # kung fu line!
    # bash -c '_PWD=\"\$PWD\"; cd \"\`echo \$0 | sed s/"$DESKTOP_NAME.desktop"//\`\"; chmod +x ./src/builders/linux/freedesktop.sh; ./src/builders/linux/freedesktop.sh --setup; ./Cerberus %F; cd \"\$_PWD\"' %k"
    ...
    local DESKTOP_FILE=(
        ...
        "Encoding=UTF-8"
        "Terminal=false"
        "Exec=bash -c '_PWD=\"\$PWD\"; cd \"\`echo \$0 | sed s/"$DESKTOP_NAME-install.desktop"//\`\"; chmod +x ./src/builders/linux/freedesktop.sh; ./src/builders/linux/freedesktop.sh --setup; ./Cerberus %F; cd \"\$_PWD\"' %k"
        "Icon=$APP_ICON_NAME"
        "Name=Linux Install"
...
do_generate_remove_launcher(){
    ...
    # kung fu line!
    # bash -c '_PWD=\"\$PWD\"; cd \"\`echo \$0 | sed s/"$DESKTOP_NAME.desktop"//\`\"; chmod +x ./src/builders/linux/freedesktop.sh; ./src/builders/linux/freedesktop.sh --uninstall; ./Cerberus %F; cd \"\$_PWD\"' %k"
    # store the current PWD, get the .desktop file path and use sed to remove the desktop name, start the installation of the icons, mime and menu, set Cerberus X and restore the PWD
    local DESKTOP_FILE=(
        "#!/usr/bin/env xdg-open"
        "[Desktop Entry]"
        ...
        "Encoding=UTF-8"
        "Terminal=false"
        "Exec=bash -c '_PWD=\"\$PWD\"; cd \"\`echo \$0 | sed s/"$DESKTOP_NAME-uninstall.desktop"//\`\"; chmod +x ./src/builders/linux/freedesktop.sh; ./src/builders/linux/freedesktop.sh --uninstall; cd \"\$_PWD\"' %k"
        "Icon=$APP_ICON_NAME"
        ...
        while [[ $# -gt 0 ]]; do
            case $1 in
                ...
                do_freedesktop_info "CERBERUS X LINUX LAUNCHER"
                    ...
                    echo -e "\t{-l|--launcher}\t\tGenerate the Cerberus X setup launcher."
